  ...
  switch (action.type) {
    ...
    case FETCH_SUCCESS_USER:
      return {
        ...
        user: {
          ...action.payload,
          inAdoption: action.payload.animals.filter(
            (animal) => animal.status === status.IN_ADOPTION
          ),
          reserved: action.payload.animals.filter(
            (animal) => animal.status === status.RESERVED
          ),
          adopted: action.payload.animals.filter(
            (animal) => animal.status === status.ADOPTED
          ),
        },
        fetch: {
          error: false,
