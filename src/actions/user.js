...
export function fetchUser(userId) {
  return (dispatch) => {
    ...
    return getUser(userId)
      ...
      .catch((error) => {
        console.log(
          'Error when retrieving the user with id ',
          userId,
          ' from db. Error: ',
          error.message
        )
...
export function fetchAnimalsUser(userId) {
  return (dispatch) => {
    ...
    return animalByUser(userId)
      .then((response) => {
        dispatch(
          getUserAnimalsSuccess(response.inAdoption, response.favourites)
        )
      })
      .catch((err) => {
        console.log(
          'Error when retrieving retrieving the animals ',
          err.message
        )
    ...
    return firebaseActions
      ...
      .then(() => {
        return deleteUserAPI(userId)
          ...
          .catch((err) => {
            console.log(
              'DB: Error when deleting the user with id ',
              userId,
              '. Error: ',
              err.message
            )
      ...
      .catch((err) => {
        console.log(
          'Firebase: Error when deleting the user with id ',
          userId,
          '. Error: ',
          err.message
        )
...
export function updateUser(user) {
  return (dispatch) => {
    ...
    return editUser(user)
      ...
      .catch((err) => {
        console.log(
          'Error when updating the user with id ',
          user._id,
          '. Error: ',
          err.message
        )
...
export function getToken() {
  return (dispatch) => {
    ...
    return getTokenAPI()
      ...
      .catch((err) => {
        console.log(`Error when retrieving the token. Error: ${err.message}`)
